...
Face.prototype.init = function (cube, type) {
	...
	this._type = type;
	this._color = null;
	this._node = OZ.DOM.elm("div", { className: "face face" + type, width: Face.SIZE + "px", height: Face.SIZE + "px", position: "absolute", top: "0px" });
...
Rubik.prototype.backMovement = function () {
	if (stack.length > 0) {
		addStack = false;
		const command = stack[stack.length - 1];
		this[command.function](-command.dir, command.layer)
		stack.pop();
		addStack = true;
	}
}

Rubik.prototype.resetAll = function () {
	const loop = () => {
		if (stack.length > 0) {
			this.backMovement();
			setTimeout(loop, 500);
...
Rubik.prototype._rotateX = function (dir, layer) {
	...
	if (addStack) {
		stack.push({ function: '_rotateX', dir: dir, layer: layer });
	}
	loading = false;
}
...
Rubik.prototype._rotateY = function (dir, layer) {
	...
	if (addStack) {
		stack.push({ function: '_rotateY', dir: dir, layer: layer });
	}
...
Rubik.prototype._rotateZ = function (dir, layer) {
	...
	if (addStack) {
		stack.push({ function: '_rotateZ', dir: dir, layer: layer });
	}
	loading = false;
}
	...
	for (var z = 0; z < Rubik.SIZE; z++) {
		for (var y = 0; y < Rubik.SIZE; y++) {
			for (var x = 0; x < Rubik.SIZE; x++) {
				...
				if (z == 2) { cube.setFace(Face.BACK, "#CA6C00"); }

				if (x == 0) { cube.setFace(Face.LEFT, "blue"); }
				if (x == 2) { cube.setFace(Face.RIGHT, "green"); }
...
Rubik.prototype.handleBack = function () {
	document.getElementById("buttonBack").addEventListener("click", () => {
		this.backMovement();
	});
}

Rubik.prototype.handleRandomize = function () {
	document.getElementById("buttonRandomize").addEventListener("click", () => {
		this.randomize();
	});
}

Rubik.prototype.handleResetAll = function () {
